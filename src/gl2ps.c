...
double gl2ps_stroke_width = 0.5;
GL2PSDLL_API void setGL2PSStrokeWidth(double _val)
{
	gl2ps_stroke_width = _val;
}
GL2PSDLL_API double getGL2PSStrokeWidth(void)
{
	return gl2ps_stroke_width;
}
...
static void gl2psPrintSVGSmoothTriangle(GL2PSxyz xyz[3], GL2PSrgba rgba[3])
{
  ...
  if(gl2psSameColorThreshold(3, rgba, gl2ps->threshold)){
   /* gl2psSVGGetColorString(rgba[0], col);
	...
	gl2psPrintf("<polygon", col);
	if (rgba[0][3] < 1.0F) gl2psPrintf(" fill-opacity=\"%g\"", rgba[0][3]);
	gl2psPrintf(" points=\"%g,%g %g,%g %g,%g\"", xyz[0][0], xyz[0][1],
		xyz[1][0], xyz[1][1], xyz[2][0], xyz[2][1]);
	gl2psPrintf(" style=\"fill:rgb(%g,%g,%g); stroke:rgb(%g,%g,%g)\" stroke-width=\"%g\"/>\n", rgba[0][0] * 255, rgba[0][1] * 255, rgba[0][2] * 255,
		rgba[0][0] * 255, rgba[0][1] * 255, rgba[0][2] * 255, gl2ps_stroke_width, col);
  }
  else{
